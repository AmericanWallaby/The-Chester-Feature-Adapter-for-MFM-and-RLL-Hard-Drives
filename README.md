...
The Arduino can be soldered to either side of the board as long as pin orientation orientation is observed. The only specialty item required is a PCB-mount card edge socket. 

The Chester PCB will accommodate both the Arduino Micro and Nano boards available at the date of publication, though the Micro is recommended. When installing, ensure that the +5V pin (not Vin) aligns with the trace leading to pin 1 of the J2 power header and USB port is oriented to the side indicated. The code should run on any board variant so long as +5v, GND, and Digital I/O pads align.
